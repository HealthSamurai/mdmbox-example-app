...
const LOCALSTORAGE_KEY = "merges-table-ui-state";

const StatusBadge = ({ status }: { status: MergeStatus }) => (
  <span
...
export function MergesTable() {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const result = await api.getMerges({ page, count, filter: filters });
  ...
  return (
    initialUiState && (
      <DataTable
        ...
        onCellClick={(row) => navigate(`/merges/${row.id}`)}
